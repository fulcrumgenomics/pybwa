...
class TestBwaIndexErrorHandling:
    ...
    def test_index_missing_dict_file(self, tmp_path: Path, test_fa: Path) -> None:
...
class TestBwaMemErrorHandling:
    ...
    def test_no_prefix_or_index(self) -> None:
    ...
    def test_align_empty_queries(self, bwa_mem_aligner: BwaMem) -> None:
        """Test alignment with empty query list."""
        result = bwa_mem_aligner.align([])
        assert result == []

    def test_align_after_exit_raises(self, test_fa: Path) -> None:
        """Test that calling align() after exiting the context manager raises RuntimeError."""
        with BwaMem(prefix=test_fa) as aligner:
            pass
        with pytest.raises(RuntimeError, match="has been closed"):
            aligner.align(["ACGT"])

    def test_context_manager(self, test_fa: Path) -> None:
...
class TestBwaAlnErrorHandling:
    ...
    def test_align_none_queries(self, bwa_aln_aligner: BwaAln) -> None:
        """Test alignment with None queries."""
        result = bwa_aln_aligner.align(queries=None)
        assert result == []

    def test_align_empty_queries(self, bwa_aln_aligner: BwaAln) -> None:
        """Test alignment with empty query list."""
        result = bwa_aln_aligner.align([])
        assert result == []

    def test_align_raw_string_rejected(self, bwa_aln_aligner: BwaAln) -> None:
        """Test that passing a bare string instead of a list raises TypeError."""
        with pytest.raises(TypeError, match="queries must be a sequence"):
            bwa_aln_aligner.align(queries="ACGTACGT")

    def test_align_quality_length_mismatch(self, bwa_aln_aligner: BwaAln) -> None:
        """Test that a quality string shorter than the sequence raises ValueError."""
        rec = FastxRecord(name="test", sequence="ACGTACGT", quality="III")
        with pytest.raises(ValueError, match="quality string length"):
            bwa_aln_aligner.align(queries=[rec])
...
class TestBwaMemOptionsValidation:
    ...
    def test_invalid_min_seed_len(self) -> None:
    ...
    def test_invalid_band_width(self) -> None:
    ...
    def test_invalid_threads(self) -> None:
    ...
    def test_invalid_max_occurrences(self) -> None:
    ...
    def test_validation_called_in_finalize(self) -> None:
...
class TestBwaAlnOptionsValidation:
    ...
    def test_invalid_max_hits(self) -> None:
...
class TestEdgeCases:
    ...
    def test_very_short_sequence(self, bwa_mem_aligner: BwaMem) -> None:
    ...
    def test_sequence_with_all_ns(self, bwa_mem_aligner: BwaMem) -> None:
    ...
    def test_options_cannot_be_modified_after_finalize(self) -> None:
        """
...
class TestUtilityErrorPaths:
    """Test error paths in shared utility functions."""

    def test_check_alloc_with_null(self) -> None:
        """Test that check_alloc raises MemoryError when given NULL."""
        from pybwa.libbwa_utils import _check_alloc_with_null

        with pytest.raises(MemoryError, match="test allocation failure"):
            _check_alloc_with_null()

    def test_check_not_null_with_null(self) -> None:
        """Test that check_not_null raises RuntimeError when given NULL."""
        from pybwa.libbwa_utils import _check_not_null_with_null

        with pytest.raises(RuntimeError, match="test null pointer"):
            _check_not_null_with_null()
